...
class TestStringMethods(unittest.TestCase):
    ...
    def test_simple_result_set_write_and_read_test(self):
        ...
        with open(test_output_file_path, 'r') as file:
            lines = [l for l in csv.reader(file, quotechar='"', delimiter=',', quoting=csv.QUOTE_MINIMAL, skipinitialspace=True)]
    ...
    def test_main_dataset_validated_test(self):
        ...
        with open(output_file_path, 'r') as output_file:
            output_file_content = [l for l in csv.reader(output_file, quotechar='"', delimiter=',', quoting=csv.QUOTE_MINIMAL, skipinitialspace=True)]

            with open(expected_result_file_path, 'r') as expected_result_file:
                expected_result_file_content = [l for l in csv.reader(expected_result_file, quotechar='"', delimiter=',', quoting=csv.QUOTE_MINIMAL, skipinitialspace=True)]
                self.assertEqual(len(expected_result_file_content), len(output_file_content), "Generated report and expected result do not have the same row count!")
                for i in range(len(output_file_content)):
                    self.assertEqual(expected_result_file_content[i], output_file_content[i], f"Line {i + 1} do not match between expected result and generated report!")
